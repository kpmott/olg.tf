...
def euler_loss(y_true,y_pred):
    ...
    #RETHINK HOW TO DO THIS WITHOUT RESHAPE COMMANDS! 
    x0 = tf.reshape(tf.concat([tf.reshape(Ω[time][0,0],(1,)),Ω[time][0,1:] + (P[0] + Δ[time][0])*ebar + bbar],axis=0),(1,L))
    ...
    # B_eul = tf.where(Cpen[:,:-1],tf.math.abs(upinv_tf(β*B_exp/Q)/C[:,:-1]-1),10000000.-C[:,:-1])
    # E_eul = tf.where(Cpen[:,:-1],tf.math.abs(upinv_tf(β*E_exp/P)/C[:,:-1]-1),10000000.-C[:,:-1])
    # B_eul = tf.where(Cpen[:,:-1],tf.math.abs(β*B_exp/Q-up(C[:,:-1])),10000000.-negC[:,:-1])
    # E_eul = tf.where(Cpen[:,:-1],tf.math.abs(β*E_exp/P-up(C[:,:-1])),10000000.-negC[:,:-1])
...
bs = 32
model.fit(tf.convert_to_tensor(Σ)[time],tf.zeros((train,output)),batch_size=bs,epochs=1,verbose=0,callbacks=[TqdmCallback()])#,tbc])

"""
skip = False
for thyme in tqdm(range(50)):
    for t in range(1):
        #t=0
        Σ = []
        Y = []
