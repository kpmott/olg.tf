...
def fit_euler(num_epochs=500,num_iters=10,tb=False):
    ...
    for thyme in tqdm(range(num_iters)):
        ...
        if tb:
            ...
            model.fit(tf.convert_to_tensor(Σ)[time],tf.zeros((train,output)),batch_size=train,epochs=num_epochs,verbose=0,callbacks=[TqdmCallback(),tbc])
        else:
            model.fit(tf.convert_to_tensor(Σ)[time],tf.zeros((train,output)),batch_size=train,epochs=num_epochs,verbose=0,callbacks=[TqdmCallback()])#,tbc])

        skip = tf.math.reduce_mean(cust_loss.euler_loss(tf.zeros((train,output)),tf.convert_to_tensor(Y,dtype='float32')[time])) <= ϵ
